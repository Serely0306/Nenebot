...
func init() {
	...
	go func() {
		...
		if !file.IsExist(bgPath) {
			err := os.MkdirAll(bgPath, 0777)
			if err != nil {
				log.Warnf("创建背景图片目录失败: %v", err)
			} else {
				log.Infof("背景图片目录已创建: %s", bgPath)
				log.Infof("请将背景图片(.jpg/.png格式)放入此目录")
			}
		}
	...
	engine.OnFullMatch("查看背景图片", zero.SuperUserPermission).SetBlock(true).Handle(func(ctx *zero.Ctx) {
		...
		if len(imageFiles) == 0 {
			ctx.SendChain(message.Text("背景图片目录为空，请将图片放入: ", bgPath))
			return
		}

		msg := "可用背景图片 (" + strconv.Itoa(len(imageFiles)) + " 张):\n"
		for i, name := range imageFiles {
			msg += strconv.Itoa(i+1) + ". " + name + "\n"
		}
...
// 修改initPic函数，使其根据群聊ID决定使用本地背景还是API
func initPic(picFile string, uid int64, gid int64) (avatar []byte, err error) {
	...
	// 检查群聊是否在使用本地背景的列表中
	if localBackgroundGroups[gid] {
		// 使用本地背景
		err = getLocalBackgroundImage(picFile)
		if err == nil {
			return avatar, nil
		}
		log.Warnf("[score] 群聊 %d 本地背景获取失败，尝试使用API: %v", gid, err)
	}

	// 不在本地背景列表中或获取本地背景失败，使用API获取
	...
	// 获取网络图片失败，使用本地已有的图片
	log.Errorf("[score] API获取背景失败: %v", err)
...
func getLocalBackgroundImage(picFile string) error {
	...
	// 检查目录是否存在
	if !file.IsExist(bgPath) {
		return errors.New("background directory not exist")
	}
	...
	if len(imageFiles) == 0 {
		return errors.New("no background images found")
	}

	// 随机选择一张图片
	selectedFile := filepath.Join(bgPath, imageFiles[rand.Intn(len(imageFiles))].Name())
