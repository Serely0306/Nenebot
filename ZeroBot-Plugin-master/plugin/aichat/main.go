...
func init() {
	...
	en.OnPrefix("设置AI聊天触发概率", zero.AdminPermission).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			...
			rate, err := strconv.ParseUint(args, 10, 8)
			if err != nil || rate > 100 {
				ctx.SendChain(message.Text("ERROR: 请输入0-100之间的数字"))
				return
			}
			...
			fs := GetFileStorage()
			if err := fs.SaveRate(gid, uint8(rate)); err != nil {
				ctx.SendChain(message.Text("ERROR: 保存失败: ", err))
				return
			}

			ctx.SendChain(message.Text("成功设置触发概率为 ", rate, "%"))
		})
	...
	en.OnPrefix("设置AI聊天温度", zero.AdminPermission).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			...
			temp, err := strconv.ParseUint(args, 10, 8)
			if err != nil || temp > 100 {
				ctx.SendChain(message.Text("ERROR: 请输入0-100之间的数字"))
				return
			}
			...
			fs := GetFileStorage()
			if err := fs.SaveTemp(gid, uint8(temp)); err != nil {
				ctx.SendChain(message.Text("ERROR: 保存失败: ", err))
				return
			}

			ctx.SendChain(message.Text("成功设置温度为 ", temp, " (实际: ", float32(temp)/100, ")"))
		})
	...
	en.OnRegex("^设置AI聊天(不)?以AI语音输出$", ensureconfig, zero.AdminPermission).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			...
			fs := GetFileStorage()
			if err := fs.SaveNoRecord(gid, isNo); err != nil {
				ctx.SendChain(message.Text("ERROR: 保存失败: ", err))
				return
			}

			if isNo {
				ctx.SendChain(message.Text("成功设置为不以AI语音输出"))
			} else {
				ctx.SendChain(message.Text("成功设置为以AI语音输出"))
			}
		})
	...
	en.OnRegex("^设置AI聊天(不)?使用Agent模式$", ensureconfig, zero.SuperUserPermission).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			...
			fs := GetFileStorage()
			if err := fs.SaveNoAgent(gid, isNo); err != nil {
				ctx.SendChain(message.Text("ERROR: 保存失败: ", err))
				return
			}

			if isNo {
				ctx.SendChain(message.Text("成功设置为不使用Agent模式"))
			} else {
				ctx.SendChain(message.Text("成功设置为使用Agent模式"))
			}
		})
	...
	en.OnRegex("^设置AI聊天(不)?响应AT$", ensureconfig, zero.OnlyPrivate, zero.SuperUserPermission).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			...
			fs := GetFileStorage()
			if err := fs.SaveNoReplyAt(gid, isNo); err != nil {
				ctx.SendChain(message.Text("ERROR: 保存失败: ", err))
				return
			}

			if isNo {
				ctx.SendChain(message.Text("成功设置为不响应@"))
			} else {
				ctx.SendChain(message.Text("成功设置为响应@"))
			}
		})
	...
	en.OnPrefix("设置AI聊天TopP", ensureconfig, zero.OnlyPrivate, zero.SuperUserPermission).SetBlock(true).
		...
		// 修改 main.go 中的配置查看命令
	en.OnFullMatch("查看AI聊天配置", ensureconfig, zero.AdminPermission).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			gid := ctx.Event.GroupID
			if gid == 0 {
				// 私聊情况
			...
			resp := fmt.Sprintf(
				"【当前AI聊天本群配置】\n"+
					...
					"• 使用Agent：%v\n"+
					"• 响应@：%v\n",
				int(stor.rate()), stor.temp(),
				ModelBool(!stor.norecord()),
				ModelBool(!stor.noagent()),
				ModelBool(!stor.noreplyat()),
			)
	...
	en.OnFullMatch("查看AI文件存储", zero.SuperUserPermission).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			gid := ctx.Event.GroupID
			if gid == 0 {
				gid = -ctx.Event.UserID
			...
			ctx.SendChain(
				message.Text(
					...
					"• 群组ID: ", gid, "\n",
					"• 触发概率: ", config.Rate, "%\n",
					"• 温度: ", config.Temp, " (实际: ", float32(config.Temp)/100, ")\n",
					"• 以AI语音输出: ", ModelBool(!config.NoRecord), "\n",
					"• 使用Agent: ", ModelBool(!config.NoAgent), "\n",
					"• 响应@: ", ModelBool(!config.NoReplyAt), "\n",
					"• 最后更新: ", config.UpdatedAt.Format("2006-01-02 15:04:05"), "\n",
				),
			)
		})
