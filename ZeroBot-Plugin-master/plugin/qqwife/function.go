...
func init() {
	// 设置互动CD（嫁娶、做媒、NTR）
	engine.OnRegex(`^设置结婚CD为(\d+)小时`, zero.OnlyGroup, zero.AdminPermission, getdb).SetBlock(true).Limit(ctxext.LimitByUser).
		Handle(func(ctx *zero.Ctx) {
			...
			if err != nil {
				ctx.SendChain(message.Text("[qqwife]设置互动CD时长失败\n", err))
				return
			}
			ctx.SendChain(message.Text("设置成功，结婚CD(嫁娶/做媒/NTR)设置为", cdTime, "小时"))
		})

	// 设置礼物CD
	engine.OnRegex(`^设置礼物CD为(\d+)小时`, zero.OnlyGroup, zero.AdminPermission, getdb).SetBlock(true).Limit(ctxext.LimitByUser).
		Handle(func(ctx *zero.Ctx) {
			cdTime, err := strconv.ParseFloat(ctx.State["regex_matched"].([]string)[1], 64)
			if err != nil {
				ctx.SendChain(message.Text("[qqwife]请设置纯数字\n", err))
			...
			err = 民政局.更新设置(groupInfo)
			if err != nil {
				ctx.SendChain(message.Text("[qqwife]设置礼物CD时长失败\n", err))
				return
			}
			ctx.SendChain(message.Text("设置成功，礼物CD设置为", cdTime, "小时"))
		})

	// 设置离婚CD
	engine.OnRegex(`^设置离婚CD为(\d+)小时`, zero.OnlyGroup, zero.AdminPermission, getdb).SetBlock(true).Limit(ctxext.LimitByUser).
		Handle(func(ctx *zero.Ctx) {
			cdTime, err := strconv.ParseFloat(ctx.State["regex_matched"].([]string)[1], 64)
			if err != nil {
				ctx.SendChain(message.Text("[qqwife]请设置纯数字\n", err))
			...
			groupInfo.DivorceCDtime = cdTime
			err = 民政局.更新设置(groupInfo)
			if err != nil {
				ctx.SendChain(message.Text("[qqwife]设置离婚CD时长失败\n", err))
				return
			}
			ctx.SendChain(message.Text("设置成功，离婚CD设置为", cdTime, "小时"))
		})
	...
	engine.OnRegex(`^设置CD为(\d+)小时`, zero.OnlyGroup, zero.AdminPermission, getdb).SetBlock(true).Limit(ctxext.LimitByUser).
		Handle(func(ctx *zero.Ctx) {
			...
			ctx.SendChain(message.Text("设置成功，互动CD(嫁娶/做媒/NTR)设置为", cdTime, "小时"))
		})
	...
	engine.OnFullMatch("qqwife设置", zero.OnlyGroup, getdb).SetBlock(true).Limit(ctxext.LimitByUser).
		Handle(func(ctx *zero.Ctx) {
			gid := ctx.Event.GroupID
			...
			ctx.SendChain(
				...
				message.Text("\n当前群qqwife设置："),
				message.Text("\n----------------"),
				message.Text("\n互动CD(嫁娶/牛头人)：", groupInfo.CDtime, "小时"),
				message.Text("\n礼物CD(买礼物/做媒)：", groupInfo.GiftCDtime, "小时"),
				message.Text("\n离婚CD(闹离婚)：", groupInfo.DivorceCDtime, "小时"),
				message.Text("\n----------------"),
				message.Text("\n", func() string {
					if groupInfo.CanMatch == 1 {
						return "允许自由恋爱"
					}
					return "禁止"
				}()),
				message.Text("\n", func() string {
					if groupInfo.CanNtr == 1 {
						return "允许牛头人"
					}
					return "禁止牛头人"
				}()),
...
func checkSingleDog(ctx *zero.Ctx) bool {
	...
	// 判断CD
	ok, err := 民政局.判断CD(gid, uid, "互动", groupInfo.CDtime)
	switch {
	case err != nil:
		ctx.SendChain(message.Text("[ERROR]:", err))
		return false
...
func checkDivorce(ctx *zero.Ctx) bool {
	...
	ok, err := 民政局.判断CD(gid, uid, "离婚", groupInfo.DivorceCDtime)
	switch {
	case err != nil:
		ctx.SendChain(message.Text("[ERROR]:", err))
		return false
...
func checkMatchmaker(ctx *zero.Ctx) bool {
	...
	ok, err := 民政局.判断CD(gid, uid, "做媒", groupInfo.GiftCDtime)
	switch {
	case err != nil:
		ctx.SendChain(message.Text("[ERROR]:", err))
		return false
