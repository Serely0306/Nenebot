...
func (f *Filter) Filter(onebotMessage *core.OneBotMessage) bool {
...
func (f *MessageTypeFilter) applyPrefixWithRule(onebotMessage *core.OneBotMessage, specificRule *SpecificRuleFilter) bool {
	...
	case MESSAGE_FORMAT_ARRAY:
		for i, message := range onebotMessage.Partial.MessageArray {
			if message.Type != MESSAGE_TYPE_TEXT {
				continue
			}
			text, _ := message.Data["text"].(string)
			index = i
			textOld = strings.TrimSpace(text)
			break
		}
	...
	if onebotMessage.Intact == nil {
		onebotMessage.Intact = make(map[string]json.RawMessage)
	}
...
func (f *MessageTypeFilter) processFilterWithRule(filterName string, onebotMessage *core.OneBotMessage, specificRule *SpecificRuleFilter) *bool {
...
func isGloballyBlocked(onebotMessage *core.OneBotMessage) bool {
	if onebotMessage == nil {
		return false
	}
	...
	if onebotMessage.Partial.MessageType == GROUP && groupID > 0 && slices.Contains(core.CONFIG.Server.Blocked.GroupIDs, groupID) {
		return true
	}
	return false
}
...
func (f *Filter) saveBlacklistToConfig(messageType string) bool {
	success := updateBotAppConfig(f.Name, func(botApp *core.BotAppsConfig) {
		switch messageType {
		case PRIVATE:
			botApp.Private.IDs = append([]int64(nil), f.Private.IDs...)
			if botApp.Private.Mode != BLACKLIST {
				botApp.Private.Mode = BLACKLIST
			}
		case GROUP:
			botApp.Group.IDs = append([]int64(nil), f.Group.IDs...)
