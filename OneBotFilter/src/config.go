...
func LoadConfigVP(path string) error {
	...
	if err := loadConfigFromDiskLocked(); err != nil {
		return err
	}
	...
	VP.OnConfigChange(func(e fsnotify.Event) {
		if consumeConfigReloadSkip() {
			log.Println("跳过内部写回触发的主配置热重载:", e.Name)
			return
		}
		log.Println("检测到主配置文件变更:", e.Name)
		if err := reloadConfigAndFilters(); err != nil {
			log.Println("重新加载主配置失败:", err)
		}
	})
...
func loadConfigFromDiskLocked() error {
...
func reloadConfigAndFilters() error {
	configMutex.Lock()
	defer configMutex.Unlock()
...
func restartPresetWatcherLocked() error {
	...
	targetDir := filepath.Join(rootDir, "config")
...
func handlePresetWatchEvent(watcher *fsnotify.Watcher, targetDir string, event fsnotify.Event) {
	...
	log.Println("检测到预设配置变更:", cleanPath)
	if err := reloadConfigAndFilters(); err != nil {
		log.Println("重新加载预设配置失败:", err)
	}
}
...
func resolveBotAppMessagesLockedWithCache(botApp *BotAppsConfig, presetCache map[string]MessageContentConfig) error {
	baseDir := filepath.Join(filepath.Dir(configPath), "config")
