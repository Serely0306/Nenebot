...
func GenerateHelpImage(cfg HelpFileConfig, fontPath string) ([]byte, error) {
...
func SaveHelpImage(cfg HelpFileConfig, fontPath, outputPath string) error {
	data, err := GenerateHelpImage(cfg, fontPath)
	if err != nil {
		return err
	}
...
func measureTotalHeight(cfg HelpFileConfig, fonts *fontSet) int {
...
func measureSectionHeight(section helpSection, fonts *fontSet) float64 {
...
func drawGradientBackground(dc *gg.Context, height int) {
	for y := 0; y < height; y++ {
		...
		r := lerp(float64(colorBgTop.R), float64(colorBgBottom.R), t)
		g := lerp(float64(colorBgTop.G), float64(colorBgBottom.G), t)
		b := lerp(float64(colorBgTop.B), float64(colorBgBottom.B), t)
...
func drawHelpContent(dc *gg.Context, cfg HelpFileConfig, fonts *fontSet) {
	y := float64(paddingTop)
	x := float64(paddingX)
...
func drawSectionCard(dc *gg.Context, x, y, w, h float64, section helpSection, fonts *fontSet) {
...
func serviceLabel(svc helpService) string {
...
func measureServiceGridHeight(services []helpService, face font.Face, totalW float64) float64 {
	if len(services) == 0 {
		return 0
	}
...
func drawServiceGrid(dc *gg.Context, x, y, totalW float64, services []helpService, fonts *fontSet) float64 {
	if len(services) == 0 {
		return 0
	}
