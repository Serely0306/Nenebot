...
from core.runtime import MSR_REPO, UPLOAD_CONFIG, VALID_REGIONS
...
def register_msr_routes(app):
    ...
    def validate_msr_access_key():
        ...
        if not UPLOAD_CONFIG.msr.access_key or UPLOAD_CONFIG.msr.access_key == "change-me":
            return jsonify({"error": "MSR access_key 尚未在配置文件中设置"}), 500
        if not access_key:
            return jsonify({"error": "请输入 access_key"}), 400
        if access_key != UPLOAD_CONFIG.msr.access_key:
