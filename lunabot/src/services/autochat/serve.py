...
async def rpc_query_embeddings(texts: list[str], model_name: str | None = None) -> list[list[float]]:
    if model_name is None:
        return await rpc_session.call('query_embedding', texts)
    try:
        return await rpc_session.call('query_embedding', texts, model_name)
    except Exception:
        # Backward-compatible with older RPC handlers that only accept `texts`.
        return await rpc_session.call('query_embedding', texts)
...
async def chat(msg: Message):
    ...
    try:
        ...
        query_embs = await rpc_query_embeddings(
            texts_to_embed,
            config.get('chat.llm.emb_model', None),
        )
...
""".strip()
        ...
        persona_cfg = config.get('chat.prompt.persona', {})
        if isinstance(persona_cfg, dict) and persona_cfg:
            if msg.group_id in persona_cfg:
                persona = persona_cfg[msg.group_id]
            else:
                persona = persona_cfg.get('default', '')
        else:
            persona = ''

        prompt_framework: str = config.get(
            'chat.prompt.framework',
            config.get('chat.llm.prompt', ''),
        )
        full_prompt: str = prompt_framework.format(
            self_id=self_id,
            self_name=self_name,
            persona=persona,
        ...
        legacy_um_id = llm_response.get('update_um_id')
        legacy_um_text = llm_response.get('update_um_text')
        if (not user_updates) and legacy_um_id and legacy_um_text:
            user_updates = [{
                'user_id': legacy_um_id,
                'profile': legacy_um_text,
            }]
            ...
            for update in user_updates:
                try:
                    ...
                    mem.um_update(
                        ...
                        max_events=config.get('chat.mem.um_max_events', 5),
                        max_names=config.get('chat.mem.um_max_names', 10),
                    )
                except Exception as e:
