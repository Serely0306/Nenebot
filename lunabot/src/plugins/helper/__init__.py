...
HELP_DOCS_PATH = str(BASE_DIR / "helps" / "{name}.md")
...
async def _(ctx: HandlerContext):
    ...
    for path in help_doc_paths:
        try:
            ...
            help_decs.append(first_parts[1] if len(first_parts) > 1 else Path(path).stem)
            help_names.append(Path(path).stem.lower())
        except:
            pass

    doc_path = None
    if args_key:
        candidate = HELP_DOCS_PATH.format(name=args_key)
        if os.path.exists(candidate):
            doc_path = candidate
        else:
            candidate = HELP_DOCS_PATH.format(name=args)
            if os.path.exists(candidate):
                doc_path = candidate
    ...
    if not doc_path:
        ...
        try:
            # 尝试从缓存读取
            name_key = Path(doc_path).stem
            doc_mtime = os.path.getmtime(doc_path)
            cache_mtime = file_db.get('help_img_cache_mtime', {})
            cache_path = create_parent_folder(f"data/helper/cache/{name_key}.png")
            if Path(cache_path).exists() and doc_mtime <= cache_mtime.get(name_key, 0):
                return await ctx.asend_reply_msg(await get_image_cq(cache_path, low_quality=True))
            else:
                logger.info(f"缓存 {name_key} 帮助文档不存在或已过期，重新渲染")
                doc_text = Path(doc_path).read_text(encoding='utf-8', errors='ignore')
                ...
                cache_mtime[name_key] = doc_mtime
