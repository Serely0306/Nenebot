...
class Gallery:
    ...
    mode: GalleryMode
    pics_dir: str
    allowed_group_ids: list[int] = field(default_factory=list)  # empty means open to all groups
...
class GalleryManager:
    ...
    def _load(self):
        ...
        for name, g in file_db.get('galleries', {}).items():
            self.galleries[name] = Gallery(
                ...
                pics_dir=g['pics_dir'],
                allowed_group_ids=[int(gid) for gid in g.get('allowed_group_ids', [])],
    ...
    def open_gall(
        ...
        pics_dir = pics_dir or GALLERY_PICS_DIR.format(name=name)
        allowed_group_ids = sorted(set(int(gid) for gid in (allowed_group_ids or [])))
        gall = Gallery(
            name=name,
            aliases=[],
    ...
    def set_gall_open_groups(self, name_or_alias: str, group_ids: list[int]) -> list[int]:
        """
        设置画廊可见群列表（空列表表示全部群可见）
        """
        g = self.find_gall(name_or_alias)
        assert g is not None, f'画廊\"{name_or_alias}\"不存在'
        g.allowed_group_ids = sorted(set(int(gid) for gid in group_ids))
        self._save()
        return g.allowed_group_ids

    def is_gall_open_for_group(self, gall: Gallery | str, group_id: int | None) -> bool:
        """
        检查画廊是否对指定群开放
        """
        if group_id is None:
            return True
        g = gall if isinstance(gall, Gallery) else self.find_gall(gall, raise_if_nofound=True)
        if not g.allowed_group_ids:
            return True
        return int(group_id) in g.allowed_group_ids
...
# 检查画廊是否对当前群开放
def assert_gall_group_open(ctx: HandlerContext, gall: Gallery):
    # 私聊下普通用户仅可访问“全部群开放”的画廊；超管私聊不受限
    if not is_group_msg(ctx.event):
        if check_superuser(ctx.event):
            return
        if gall.allowed_group_ids:
            raise ReplyException(f'画廊\"{gall.name}\"不存在')
        return
    if not GalleryManager.get().is_gall_open_for_group(gall, ctx.group_id):
        raise ReplyException(f'画廊\"{gall.name}\"未对本群开放')
...
gall_access = CmdHandler([
    '/gall access', '/gall group',
], logger)
gall_access.check_cdrate(cd).check_wblist(gbl).check_superuser()
@gall_access.handle()
async def _(ctx: HandlerContext):
    HELP = """
使用方式:
/gall access 画廊名                   查看当前开放群
/gall access 画廊名 all               对全部群开放
/gall access 画廊名 set 群号...       仅对指定群开放
/gall access 画廊名 add 群号...       增加可访问群
/gall access 画廊名 del 群号...       删除可访问群
说明:
- 群号支持 this/here/当前群（仅群聊中可用）
- 空列表表示“全部群开放”
""".strip()
    ...
    g = mgr.find_gall(args[0], raise_if_nofound=True)

    def parse_group_ids(tokens: list[str]) -> list[int]:
        if not tokens:
            raise ReplyException(HELP)
        group_ids = []
        for token in tokens:
            if token in {'this', 'here', '当前群'}:
                assert_and_reply(ctx.group_id, '当前不在群聊中，请直接填写群号')
                group_ids.append(int(ctx.group_id))
                continue
            try:
                gid = int(token)
                assert gid > 0
                ...
                raise ReplyException(f'群号\"{token}\"无效')
        return sorted(set(group_ids))

    def render_status(groups: list[int]) -> str:
        if not groups:
            return f'画廊\"{g.name}\"当前对全部群开放'
        return f'画廊\"{g.name}\"当前仅对以下群开放:\n' + '\n'.join(str(gid) for gid in groups)

    if len(args) == 1:
        return await ctx.asend_reply_msg(render_status(g.allowed_group_ids))

    op = args[1].lower()
    if op in {'show', 'list'}:
        return await ctx.asend_reply_msg(render_status(g.allowed_group_ids))
    if op in {'all', 'clear'}:
        mgr.set_gall_open_groups(g.name, [])
        return await ctx.asend_reply_msg(render_status([]))

    if op == 'set':
        groups = parse_group_ids(args[2:])
        groups = mgr.set_gall_open_groups(g.name, groups)
        return await ctx.asend_reply_msg(render_status(groups))

    if op in {'add', 'del', 'remove'}:
        target = set(parse_group_ids(args[2:]))
        current = set(g.allowed_group_ids)
        if op == 'add':
            if not current:
                current = target
            else:
            ...
            if not current:
                raise ReplyException('当前为全部群开放，请先使用 set 指定可访问群')
            current.difference_update(target)
        groups = mgr.set_gall_open_groups(g.name, sorted(current))
        return await ctx.asend_reply_msg(render_status(groups))
...
gall_init = CmdHandler([
    '/gall init',
], logger)
gall_init.check_cdrate(cd).check_wblist(gbl).check_superuser()
@gall_init.handle()
async def _(ctx: HandlerContext):
    root_dir = ctx.get_args().strip() or 'data/gallery'
    if not os.path.isdir(root_dir):
        raise ReplyException(f'目录\"{root_dir}\"不存在')
    ...
    if not folders:
        return await ctx.asend_reply_msg(f'目录\"{root_dir}\"下没有可初始化的画廊目录')
    ...
    for name, folder in folders:
        try:
            if mgr.find_gall(name):
                skip_list.append(name)
                continue
            mgr.open_gall(name, pics_dir=folder)
            new_pids, del_pids = await mgr.async_reload_gall(name)
            ok_list.append(f'{name}(+{len(new_pids)}/-{len(del_pids)})')
        except Exception as e:
            logger.warning(f'初始化画廊\"{name}\"失败: {get_exc_desc(e)}')
            err_list.append(name)

    msg = f'画廊初始化完成\n目录: {os.path.abspath(root_dir)}'
    msg += f'\n成功: {len(ok_list)}个'
    msg += f'\n跳过(已存在): {len(skip_list)}个'
    msg += f'\n失败: {len(err_list)}个'
    if ok_list:
        msg += '\n\n成功列表:\n' + '\n'.join(ok_list)
    if skip_list:
...
""".strip()
    ...
    try: 
        ...
        pics = [mgr.find_pic(pid, raise_if_nofound=True) for pid in pids]
        ...
        if '-' in args:
            ...
            pics = mgr.find_gall(args.strip(), raise_if_nofound=True).pics
            if len(pics) < nindex:
    ...
    for name in names:
        g = mgr.find_gall(name, raise_if_nofound=True)
        assert_gall_group_open(ctx, g)
...
async def _(ctx: HandlerContext):
    ...
    # 列出所有画廊
    if not name:
        galls = mgr.get_all_galls()
        if is_group_msg(ctx.event):
            galls = {name: g for name, g in galls.items() if mgr.is_gall_open_for_group(g, ctx.group_id)}
        elif not is_super:
            galls = {name: g for name, g in galls.items() if not g.allowed_group_ids}
        if not galls:
            return await ctx.asend_reply_msg('当前没有可查看的画廊')
        
        with Canvas(bg=FillBg(THUMBNAIL_BG_COLOR)).set_padding(8) as canvas:
            with Grid(row_count=int(math.sqrt(len(galls))), hsep=8, vsep=8).set_item_align('t').set_content_align('t'):
                for name, g in galls.items():
                    cover: GalleryPic = mgr.find_pic(g.cover_pid or 0)
