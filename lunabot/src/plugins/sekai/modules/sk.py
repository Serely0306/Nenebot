...
# 从榜线数据解析Rankings
async def parse_rankings(ctx: SekaiHandlerContext, event_id: int, data: dict, data_top100: dict) -> List[Ranking]:
    #data_top100 = data_top100.get('top100', {})
...
async def get_latest_ranking(ctx: SekaiHandlerContext, event_id: int, query_ranks: List[int] = ALL_RANKS) -> List[Ranking]:
    ...
    urltop100 = get_gameapi_config(ctx).ranking_api_url.replace("border", "top100")
    assert_and_reply(url, f"暂不支持获取{ctx.region}榜线数据")
    data = await request_gameapi(url.format(event_id=event_id % 1000))
    datatop100 = await request_gameapi(urltop100.format(event_id=event_id % 1000))
    assert_and_reply(data, "获取榜线数据失败")
    assert_and_reply(datatop100, "获取榜线Top100数据失败")
    logger.info(f"从API获取 {ctx.region}_{event_id} 最新榜线数据")
    return [r for r in await parse_rankings(ctx, event_id, data, datatop100) if r.rank in query_ranks]
