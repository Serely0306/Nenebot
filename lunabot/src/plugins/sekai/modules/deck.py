...
async def extract_target_event(
    ...
    event_ctx = ctx
    fallback_ctx = SekaiHandlerContext.from_region("jp") if ctx.region != "jp" else None
    ...
    if event_id is None:
        ...
        if not event and fallback_ctx:
            fallback_event = await get_current_event(fallback_ctx, "next")
            if fallback_event:
                event = fallback_event
                event_ctx = fallback_ctx
...
""".strip())
        ...
        if not event and fallback_ctx:
            fallback_event = await fallback_ctx.md.events.find_by_id(event_id)
            if fallback_event:
                event = fallback_event
                event_ctx = fallback_ctx
    ...
    活动{event_ctx.region}-{event_id}不存在，可以指定团队+颜色进行模拟活动组卡
    """.strip())

    # 获取WL章节
    wl_cid = None
    wl_events = await get_wl_events(event_ctx, event['id']) if event_id != 180 else []
    if wl_events:
        if not chapter_id and not chapter_nickname:
            ...
            assert_and_reply_or_return(chapter, f"活动 {event_ctx.region}-{event['id']} 没有章节 {chapter_id}")
        else: 
            cid = get_cid_by_nickname(chapter_nickname)
            chapter = find_by(wl_events, "wl_cid", cid)
            assert_and_reply_or_return(chapter, f"活动 {event_ctx.region}-{event['id']} 没有 {chapter_nickname} 的章节 ")

        wl_cid = chapter['wl_cid']

    else:
        # 指定 wlx 的情况报错
        assert_and_reply_or_return(not chapter_id, f"活动 {event_ctx.region}-{event['id']} 不是WL活动，无法指定章节")
        # 指定角色昵称的情况不报错，直接忽略
        wl_matched_texts = []
...
def log_options(ctx: SekaiHandlerContext, user_id: int, options: DeckRecommendOptions):
    ...
    log = "组卡配置: "
    log += f"region={options.region or ctx.region}, "
    log += f"uid={user_id}, "
...
async def do_deck_recommend_batch(
    ...
    user_data: bytes,
    recommend_region: Optional[str] = None,
) -> list[Tuple[DeckRecommendResult, List[str], Dict[str, Tuple[timedelta, timedelta]]]]:
    recommend_region = recommend_region or ctx.region
    # 获取组卡后端相关信息
    ...
    recommend_data = { 'region': recommend_region, 'batch_options': [] }
    original_indices = []
...
async def compose_deck_recommend_image(
    ...
    original_userhonors = profile.get('userHonors', [])
    # 组合卡牌过滤
...
""".strip())
    ...
    # 准备用户数据
    if profile.get('userHonors'):
        recommend_ctx = ctx if recommend_region == ctx.region else jp_ctx
        honor_ids = {honor['id'] for honor in await recommend_ctx.md.honors.get()}
        profile['userHonors'] = [
            honor for honor in profile['userHonors']
            if honor.get('honorId') in honor_ids
        ]
    user_data = dump_bytes_json(profile)  
    # 还原profile避免画头像问题
    profile['userCards'] = original_usercards
    profile['userHonors'] = original_userhonors
    ...
    result_decks = []
    result_algs = []
    for res, algs, cost_and_wait_times in await do_deck_recommend_batch(ctx, all_options, user_data, recommend_region):
    ...
    if recommend_type in ["event", "wl", "bonus", "wl_bonus", "mysekai"] and event_id:
        ...
        if event is None:
            event = await event_ctx.md.events.find_by_id(event_id)
        if event:
            event_banner = await get_event_banner_img(event_ctx, event)
