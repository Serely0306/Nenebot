    ...
    async def __aenter__(self) -> Page:
        ...
        if _playwright_browser is None or not _playwright_browser.is_connected():
            ...
            if _playwright_instance is None: 
                ...
                pass
            # Skip temp cleanup on Windows
            if os.name != "nt":
                if os.system("rm -rf /tmp/rust_mozprofile*") != 0:
                    utils_logger.error("Failed to clear WebDriver temp files")
            #启动浏览器
...
.markdown-body {
    font-family: "Source Han Sans", "Noto Sans CJK SC", "Microsoft YaHei", Arial, sans-serif;
    color: #1f2328;
    line-height: 1.6;
    font-size: 16px;
...
async def markdown_to_image(markdown_text: str, width: int = 600) -> Image.Image:
    ...
    async with PlaywrightPage() as page: # WebDriver 返回 Playwright Page
        ...
        css_path = base_dir / "data" / "utils" / "m2i" / "m2i.css"
        if css_path.exists():
            css_content = css_path.read_text(encoding='utf-8', errors='ignore')
        else:
            css_content = DEFAULT_M2I_CSS
        try:
            import mistune
