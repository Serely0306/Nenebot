...
### 查询PJSK详情
`/pjskdetail` `/盒`
> 查询综合详情图，会汇总资源、材料、当前编组与卡面统计、加成信息、挑战Live信息、队长次数等，需要📡抓包数据  
支持服务器: `所有`
...
### 查询材料信息
`/材料信息` `/pjsk material` `/pjsk材料`
> 查询当前账号的普通材料与金币持有情况，需要📡抓包数据  
默认隐藏数量为0的材料，添加`all`参数可以查看所有历史获取记录  
支持服务器: `所有`
...
### 查询角色等级任务（测试入口）
`/cr任务测试` `/crtest`
> 独立测试版CR任务查询，不占用现有`/角色等级任务`；需要📡抓包数据
默认查询该角色任务总览，添加`all`参数并指定任务名可查询该任务的全量档位、累计需求和累计EXP
支持服务器: `所有`

- `/cr任务测试 miku`
- `/cr任务测试 miku all 队长次数`
- `/cr任务测试 miku all 衣装`
...
使用 `/难度排行` `/定数表` 或者加上 `定数` 参数时显示基于定数表的难度排行     
支持服务器: `所有`  
...
Live类型: `单人` `多人` `auto`      
比较依据: `score` `pt` `pt/h`      
指定技能组(多人指定的是实效): `90 80 70 60 50` `270`      
指定技能顺序选择策略: `最优` `最差` `平均`          
指定综合: `综合30w`     
指定活动加成: `加成430`      
指定游玩间隔: `间隔45` (间隔45s约等于虾30周回)      
筛选难度: `<29` `=30` `>28` `>=20`     
筛选等级: `expert` `ma apd`    
关注歌曲: `虾ex` `龙ma`   
支持服务器: `jp`   
...
- `/歌曲排行 单人 90 80 70 60 50 平均` 指定技能组并选择平均技能顺序
- `/歌曲排行 多人 270 pt/h 综合37w 加成430 间隔30` 多人，比较依据为pt/时间效率，实效270以及综合、加成、间隔
...
新增参数: `mr排序` / `slv排序` / `获取时间排序`  
添加 `获取时间` / `入手时间` / `obat` 可切换为获取时间视图，仅支持单角色卡牌查询  
获取时间视图下可继续添加 `按年` / `year` 按年份分组  
添加 `技能类型` / `技能分类` 可切换为技能类型视图，仅支持单角色卡牌查询
添加 `技能值` / `按技能值` / `按加分值` 可切换为技能值视图，仅支持单角色卡牌查询
支持服务器: `所有`  
...
- **冲榜备战**
    ...
    - **备战模式** `备战` `战备`
        - 依附于活动PT类组卡结果，只返回最优卡组，计算单局PT、时速、局数、耗时和火耗
        - 必须指定火耗，例如 `5火`
        - 必须指定目标PT，例如 `目标1000w` `目标300万`
        - 可选指定当前PT，例如 `今300w` `现300w` `现在300w`
        - 可选指定周回数，例如 `28周回`
        - 不指定周回数时，会按当前歌曲和live类型自动使用理论最大周回/h
        - 暂不支持和`歌曲比较`同时使用
...
- `/组卡 当前 备战 目标1000w 5火 28周回` 使用当前主队计算冲榜备战数据
- `/组卡 vs 橙 单人 备战 今300w 目标1000w 5火` 计算从当前PT打到目标PT的备战数据
...
### 心愿榜个人查询
`/wishsk`
> 查询自己、@用户，或直接查询国服心愿榜前500名次的排名、积分、500线和最近一次同步差值  
仅支持国服，且需要绑定国服账号  
支持服务器: `cn`
...
### 查询烤森材料
`/烤森材料` `/mysekai材料` `/pjsk mysekai材料`
> 查询烤森材料持有情况，需要📡抓包数据  
默认隐藏数量为0的材料，添加 `all` 参数可查看所有历史记录  
支持服务器: `jp` `cn`
...
### 切换MSR限制ID
`/msr换绑`
> 将当前群内 MSR 查询限制ID 切换为你当前绑定的ID  
需要二次确认，默认一周内只能切换一次；超管可追加 `force` 强制切换  
支持服务器: `cn`
