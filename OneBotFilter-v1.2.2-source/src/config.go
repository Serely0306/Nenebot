...
type MessageContentConfig struct {
	...
	Filters       []string                      `mapstructure:"filters" yaml:"filters"`               // 默认规则
	Prefix        []string                      `mapstructure:"prefix" yaml:"prefix"`                 // 默认前缀
	PrefixReplace string                        `mapstructure:"prefix-replace" yaml:"prefix-replace"` // 默认前缀替换
	SpecificRules map[string]SpecificRuleConfig `mapstructure:"specific-rules" yaml:"specific-rules"` // 新增：ID特定规则
}
...
type SpecificRuleConfig struct {
	...
	Mode          string   `mapstructure:"mode" yaml:"mode"` // 可选，如果不设置则使用上级的mode
}
...
func (bac *BotAppsConfig) Check() error {
	...
	if bac.Private.Message.SpecificRules != nil {
		for idStr, rule := range bac.Private.Message.SpecificRules {
			// 验证ID格式
			if _, err := strconv.ParseInt(idStr, 10, 64); err != nil {
				return fmt.Errorf("%s.private.message.specific-rules.%s: ID格式错误", bac.Name, idStr)
			}
			// 验证规则模式
			if rule.Mode != "" && rule.Mode != ON && rule.Mode != WHITELIST && rule.Mode != BLACKLIST {
				return fmt.Errorf("%s.private.message.specific-rules.%s.mode配置错误，只能是 on、whitelist 或 blacklist", bac.Name, idStr)
			}
		}
	}
	...
	if bac.Group.Message.SpecificRules != nil {
		for idStr, rule := range bac.Group.Message.SpecificRules {
			if _, err := strconv.ParseInt(idStr, 10, 64); err != nil {
				return fmt.Errorf("%s.group.message.specific-rules.%s: ID格式错误", bac.Name, idStr)
			}
			if rule.Mode != "" && rule.Mode != ON && rule.Mode != WHITELIST && rule.Mode != BLACKLIST {
				return fmt.Errorf("%s.group.message.specific-rules.%s.mode配置错误，只能是 on、whitelist 或 blacklist", bac.Name, idStr)
			}
		}
	}
	return nil
