(function () {
    ...
    function ensureActionMessage() {
        ...
        panel.insertBefore(el, tabs.nextSibling);
        return el;
    }

    function showActionMessage(text, type) {
        const el = ensureActionMessage();
        ...
        if (type === 'success') {
            el.style.background = 'rgba(68,199,180,0.10)';
            el.style.color = '#2d8f82';
            el.style.borderColor = 'rgba(68,199,180,0.22)';
        } else {
            el.style.background = 'rgba(208,96,96,0.08)';
            el.style.color = '#d06060';
            el.style.borderColor = 'rgba(208,96,96,0.18)';
        }
    ...
    async function readErrorMessage(resp, fallback) {
        const defaultText = fallback || ('请求失败（HTTP ' + resp.status + '）');
        try {
    ...
    async function approve(id) {
        ...
        if (resp.ok) {
            await loadList();
            showActionMessage('已通过申请', 'success');
            return;
        }
        showActionMessage(await readErrorMessage(resp, '通过申请失败'), 'error');
    }
    async function reject(id) {
        ...
        if (resp.ok) {
            await loadList();
            showActionMessage('已拒绝申请', 'success');
            return;
        }
        showActionMessage(await readErrorMessage(resp, '拒绝申请失败'), 'error');
    }
    async function revokeApplication(appId, btn) {
        ...
        btn.textContent = '撤销中...';
        try {
            const resp = await api('/api/apply/' + appId + '/revoke', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (resp.ok) {
                await loadList();
                showActionMessage('已撤销到待审核状态', 'success');
                return;
            }
            showActionMessage(await readErrorMessage(resp, '撤销失败'), 'error');
        } catch (e) {
            console.error('撤销失败:', e);
            showActionMessage('网络错误，请重试', 'error');
        } finally {
            btn.disabled = false;
    ...
    async function editNote(appId, btn) {
        ...
        btn.textContent = '...';
        try {
            const resp = await api('/api/apply/' + appId + '/note', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ admin_note: note.trim() }),
            });
            ...
            if (resp.ok) {
                await loadList();
                showActionMessage('备注已更新', 'success');
                return;
            }
            showActionMessage(await readErrorMessage(resp, '编辑备注失败'), 'error');
        } catch (e) {
    ...
    async function deleteRecord(appId, btn) {
        ...
        btn.textContent = '...';
        try {
            const resp = await api('/api/apply/' + appId + '/delete', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            });
            if (!resp) return;
            if (resp.ok) {
                await loadList();
                showActionMessage('申请记录已删除', 'success');
                return;
            }
            showActionMessage(await readErrorMessage(resp, '删除失败'), 'error');
        } catch (e) {
