...
from core.runtime import LUNABOT_DATA_BASE, REGION_NAMES, VALID_REGIONS
from sekai.services.data import load_and_filter_json

bp = Blueprint("sekai_bot_api", __name__, url_prefix="/api")


@bp.route("/mysekai/<region>/<uid>", methods=["GET"])
...
@bp.route("/suite/<region>/<uid>", methods=["GET"])
...
@bp.route("/mysekai/<region>/upload_times", methods=["GET", "POST"])
...
@bp.route("/mysekai/<region>/msr_sub", methods=["PUT"])
def update_msr_sub(region):
    region = region.lower()
    uid_modes = request.get_json()
    print(f"更新 {REGION_NAMES.get(region, region)} MSR 订阅: {len(uid_modes or [])} 个用户")
